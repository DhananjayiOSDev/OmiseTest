...
class AlertViewController: UIView {
    ...
    @IBOutlet weak var negBtnView: UIView!

    @IBOutlet weak var successNotifView: UIView!
    ...
    required init?(coder aDecoder: NSCoder) {
        ...
        self.frame = CGRect(x: 0, y: 0, width: OTConstants.SCREEN_SIZE.width, height: OTConstants.SCREEN_SIZE.height)
    }
...
//        }
    ...
    func config(title: String, message: String, animationType: AlertAnimationType = .scale,CancelButton isCancel:Bool) -> AlertViewController {
        if isCancel == false {
            ...
            DispatchQueue.main.async(execute: {
                ...
                if self.negBtnView != nil {
                    self.negBtnView.isHidden = true;
                }
            });
        }
    ...
    func configSuccessNotif() ->AlertViewController {
        alertView.isHidden = true
        successNotifView.isHidden = false

        return self
    }

    func showSuccessNotifView(into view:UIView) {
    ...
    func configUI(){
        ...
        self.titleLabel.textColor = UIColor.appThemeColor
